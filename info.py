...
auth_channel = environ.get('-1002457749160')
auth_grp = environ.get('-1002354320410')
...
REQ_CHANNEL = int(environ.get('-1002281356201'))
PRIME_MEMBERS_LOGS = int(environ.get('PRIME_MEMBERS_LOGS'))

# PREMIUM ACCESS *
lazydownloaders = [int(lazydownloaders) if id_pattern.search(lazydownloaders) else lazydownloaders for lazydownloaders in environ.get('PRIME_DOWNLOADERS', '6283322330').split()]
PRIME_USERS = (lazydownloaders) if lazydownloaders else [] # users who can get & download file without url shortner
lazy_renamers = [int(lazrenamers) if id_pattern.search(lazrenamers) else lazrenamers for lazrenamers in environ.get('LAZY_RENAMERS', '6283322330').split()]
...
LZURL_PRIME_USERS = [int(lazyurlers) if id_pattern.search(lazyurlers) else lazyurlers for lazyurlers in environ.get('LZURL_PRIME_USERS', '6283322330').split()]
...
ADMIN_USRNM = environ.get('ADMIN_USRNM','Cr3atives_Cortex') # WITHOUT @
MAIN_CHANNEL_USRNM = environ.get('MAIN_CHANNEL_USRNM','Cr3atives_Cortex') # WITHOUT @
DEV_CHANNEL_USRNM = environ.get('DEV_CHANNEL_USRNM','Cr3atives_Cortex') # WITHOUT @
LAZY_YT_HANDLE = environ.get('LAZY_YT_HANDLE','Cr3atives_Cortex')  # WITHOUT @ [  add only handle - don't add full url  ] 
MOVIE_GROUP_USERNAME = environ.get('MOVIE_GROUP_USERNAME', "+xP3DOP6TvHQ0NzI9") #[ without @ ]
...
my_users = [int(my_users) if id_pattern.search(my_users) else my_users for my_users in environ.get('MY_USERS', '6283322330').split()]
